...
import 'package:chess_game/server_side/play_with_friends.dart';
...
class PlayOnline3 extends StatefulWidget {
  final int? initialTime;
  const PlayOnline3({super.key, this.initialTime});

  @override
  State<PlayOnline3> createState() => _PlayOnline2State();
}

class _PlayOnline2State extends State<PlayOnline3>with TickerProviderStateMixin {
  int activeIndex = 0;
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const PlayWithFriends(bettingAmount: '', userData: {},)
            ),
          );
        }else {
          // Handle error
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    ),Text('WITH ',style: GoogleFonts.oswald(color: Colors.white,fontSize: screenWidth/20,fontWeight: FontWeight.bold),
                    ),
                    Text('FRIENDS',style: GoogleFonts.oswald(fontSize: screenWidth/20,fontWeight: FontWeight.bold,color: Colors.amberAccent),),
                  ],
                ),
                Padding(
