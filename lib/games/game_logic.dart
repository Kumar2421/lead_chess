...
class GameLogicImplementation extends GameLogic {
  ...
  GameLogicImplementation()
   ...
   //  chessHistory.add({
   //    'fen': chess.fen,
   //    'eatenBlack': List<PieceType>.from(eatenBlack),
   //    'eatenWhite': List<PieceType>.from(eatenWhite),
   //  });

    // _player1Timer.start(_onTimerTick, _onTimerFinished);     // player open the game start time
  }
  ...
  void sendGameStateToServer() {
    SharedPreferences.getInstance().then((prefs) {
      ...
      if (gameMode == 'online') {
        ...
        Future.forEach(websocketUrls, (url) async {
          try {
            ...
            void sendUpdate(String sessionToken, String name, String opponent) {
              final gameState = {
                'fen': chess.fen,
              ...
              print('Sent game state update to server at $url: $message');
            }

            sendUpdate(sessionToken, name, opponent);

            webSocket.listen((message) {
              print('Message received from server at $url: $message');
              final decodedMessage = jsonDecode(message);
              if (decodedMessage['type'] == 'game_state') {
                handleReceivedGameState(decodedMessage['game_state']);
              }
            }).onDone(() {
              print('WebSocket connection to server at $url closed.');
            });
          } catch (e) {
            print('Error connecting to server at $url: $e');
          }
        }).then((_) {
          print('Game state sent to all servers.');
        }).catchError((error) {
          print('Error sending game state to servers: $error');
        });
      }
    });
  }
  ...
  void handleReceivedGameState(Map<String, dynamic> gameState) {
    ...
    // final List<Map<String, dynamic>>? history = gameState['history'] != null
    //     ? List<Map<String, dynamic>>.from(gameState['history'] as List<dynamic>)
    //     : null;
    // final List<Map<String, dynamic>>? redoStack = gameState['redoStack'] != null
    //     ? List<Map<String, dynamic>>.from(gameState['redoStack'] as List<dynamic>)
    //     : null;
    ...
    // Save the received game state locally
    Game game = Game.fromGameState(fen, args, this.eatenBlack, this.eatenWhite);
    game.save();
  ...
  void tapTile(String index) {
    ...
    // Check the game mode from shared preferences
    SharedPreferences.getInstance().then((prefs) {
      String? gameMode = prefs.getString('game_mode');
      if (gameMode == 'online') {
        print('$gameMode');
        ...
        if (isAnyTimerFinished()) {
          return; // Do not allow moves if the game time is over
        }
        if ((args.asBlack && turn() != PieceColor.BLACK) ||
            (!args.asBlack && turn() != PieceColor.WHITE)) {
          return; // Not the player's turn, so return early
        }
        ...
        if (selectedTile == null) {
          ...
          Piece? selectedPiece = chess.get(index);
          if (selectedPiece != null &&
              ((args.asBlack && selectedPiece.color == PieceColor.BLACK) ||
                  (!args.asBlack && selectedPiece.color == PieceColor.WHITE))) {
            // Select the piece if it belongs to the player
            select(index);
          }
        } else {
          // A piece is already selected
          Piece? targetPiece = chess.get(index);
          if (targetPiece == null || targetPiece.color != turn()) {
            // Either the target square is empty or has an opponent's piece (valid move)
            makeMove(selectedTile!, index);
            select(null); // Deselect after move
          } else {
            // The target square has a piece of the same color (select a new piece)
            select(index);
          }
        }
        // Additional logic for online multiplayer
        // For example: Handle online multiplayer moves, updates, etc.
      } else {
        print('$gameMode: ----------------- ');
        if (gameOver()) {
          return;
