...
class Game {
  ...
  factory Game.fromGameState(String fen, GameArguments args, List<PieceType> eatenBlack, List<PieceType> eatenWhite) {
    return Game(
      id: Localstore.instance.collection("games").doc().id,
      name: DateTime.now().toString().substring(0, 16) + (args.isMultiplayer ? " Multiplayer" : " vs ${args.difficultyOfAI}"),
      fen: fen,
      args: args,
  ...
  Future<void> save() async {
    await Localstore.instance.collection('games').doc(id).set(toMap());
  }

  Future<void> delete() async {
    await Localstore.instance.collection('games').doc(id).delete();
  }
}
